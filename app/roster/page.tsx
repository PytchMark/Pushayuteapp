...
const RosterPage = () => {
  ...
  const categories = ['All', 'DJ', 'Band', 'Singer', 'Host', 'Poet', 'Dancer', 'Instrumentalist'];

  const filteredTalents = useMemo(() => {
    if (filter === 'All') return demoTalents;
    return demoTalents.filter((talent) => talent.categories.includes(filter));
  }, [filter]);
    ...
    <div className="max-w-7xl mx-auto px-6 py-40">
      <header className="mb-24 flex flex-col md:flex-row md:items-end justify-between gap-10">
        <div className="max-w-2xl">
          ...
          <p className="text-zinc-500 text-lg md:text-xl font-light tracking-wide leading-relaxed">
            Our talent roster is a curated list of performers represented through Pytch Marketing.
            Every artist here is professionally vetted for the stage.
          </p>
        </div>
      ...
      {filteredTalents.length === 0 && (
        <div className="py-40 text-center">
          <p className="text-xl uppercase tracking-[0.3em] font-black text-zinc-700">
